...
def print_days(days_in_month, start_day):
    ...
    if (days_in_month + start_day) % 7 != 0:  #  пустая строка печатается только в случае,
        print()                                   # если последний день месяца - не воскресенье
...
def get_starting_day(year):       # Сторонняя программа
    d = 1
    m = 13
    y = year - 1
